...
namespace Enigma.D3.Helpers
{
	public static class UXHelper
	{
		...
		public static IEnumerable<UXItemsControl> Enumerate()
		{
			var map = GetUIMap();
			if (map == null)
			...
			foreach (var entry in map)
			{
				...
				{
					var control = entry.x10_PtrComponent.Cast<UXItemsControl>().Dereference();
		...
		{
			return Engine.TryGet(engine => engine.ObjectManager.UI.Dereference().x0000_Ptr_Controls.Dereference().x10_Map);
		}
